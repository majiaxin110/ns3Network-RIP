...
//  listRH.Add (staticRh, 5);
  ...
  //!! Here is the Key to set concrete static position
  ListPositionAllocator mylistPositionAllocator;
  mylistPositionAllocator.Add(srcPosition);
  mylistPositionAllocator.Add(destPosition);
  ...
  ListPositionAllocator routerPositionAllocator;
  for(int i=40;i<71;i = i+10) {
	  Vector pos(50,i,0);
	  if(i >= 60)
		  pos = Vector(50 + i - 50,45,0);
	  routerPositionAllocator.Add(pos);
  }
  MobilityHelper routerMobility;
  routerMobility.SetPositionAllocator(&routerPositionAllocator);
  routerMobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
  routerMobility.Install(routers);
  ...
  AnimationInterface anim("xmls/rip-simple.xml");
  ...
  anim.UpdateNodeColor(src,0,255,255);
